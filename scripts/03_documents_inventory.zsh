#!/bin/zsh
# LifeOS Organizer — Task 03
# Metadata-only inventory for the configured Documents target.
...
REPORT_PATH="$PROJECT_DIR/reports/03_documents_inventory.txt"
# Execution lock: prevents concurrent Task 03 runs from writing to shared
# output paths. Root cause of prior artifact corruption (see
# reports/02_5_process_diagnostics.txt): repeated manual invocations before
# earlier terminal-backed runs had exited.
LOCK_DIR="$PROJECT_DIR/logs/.task03.lock"
# Spotlight enrichment is optional and disabled by default to keep large inventories bounded.
# Run COLLECT_SPOTLIGHT=1 ./scripts/03_documents_inventory.zsh only when separately approved.
...
TARGET_PATH="$(/usr/bin/awk '
  $0 == "Documents:" { found=1; next }
  found && $1 == "path:" { print substr($0, index($0, ":") + 2); exit }
  found && /^[^[:space:]]/ { exit }
' "$CONFIG_PATH")"

if [[ -z "$TARGET_PATH" || ! -d "$TARGET_PATH" ]]; then
  print -u2 -- 'ERROR: Configured Documents target is unavailable.'
  exit 1
fi
...
if ! /bin/mkdir "$LOCK_DIR" 2>/dev/null; then
  ...
  if [[ -n "$holder_pid" ]] && /bin/kill -0 "$holder_pid" 2>/dev/null; then
    print -u2 -- "ERROR: Another Task 03 run (PID $holder_pid) holds the execution lock ($LOCK_DIR). Refusing to run concurrently. Let it finish or terminate it first."
    exit 1
  fi
...
# collide (prior bug: hardcoded "-001" caused two different runs to both
# publish as INV-20260801-001, breaking audit-trail uniqueness).
INVENTORY_ID="INV-$(/bin/date '+%Y%m%d')-$(/bin/date '+%H%M%S')"
RUN_DIR="$(/usr/bin/mktemp -d "$OUTPUT_DIR/.task03.XXXXXX")" || { /bin/rm -rf "$LOCK_DIR"; print -u2 -- 'ERROR: Cannot create repository-local staging directory.'; exit 1; }
CSV_PATH="$RUN_DIR/metadata.csv"
JSON_PATH="$RUN_DIR/metadata.json"
SUMMARY_PATH="$RUN_DIR/summary.md"
REPORT_PATH="$RUN_DIR/03_documents_inventory.txt"
...

{
  ...
  print -- "- Engine: Task 03 Documents metadata inventory"
  print
  section 'Folder Path'
  print -- "$TARGET_PATH"
  print
...
{
  print -- 'LifeOS Organizer — Documents Metadata Inventory'
  print -- "Inventory ID: $INVENTORY_ID"
  print -- "Timestamp: $TIMESTAMP"
...
/bin/mv "$REPORT_PATH" "$PROJECT_DIR/reports/03_documents_inventory.txt" || exit 1
