#!/bin/zsh
# LifeOS Organizer — Task 05
# Metadata-only inventory for the configured Downloads target.
# This script never opens user documents or follows symlinks.
# Cloned from the validated scripts/04_desktop_inventory.zsh (see DECISIONS.md
# D1-D7) after that pattern was proven end-to-end on 2026-08-02.
...
REPORT_PATH="$PROJECT_DIR/reports/05_downloads_inventory.txt"
# Execution lock: prevents concurrent Task 05 runs from writing to shared
# output paths. Same pattern as Tasks 03/04 (see DECISIONS.md D5/D6/D7),
# applied here from the start rather than retrofitted after an incident.
LOCK_DIR="$PROJECT_DIR/logs/.task05.lock"
# Spotlight enrichment is optional and disabled by default to keep large inventories bounded.
# Run COLLECT_SPOTLIGHT=1 ./scripts/05_downloads_inventory.zsh only when separately approved.
...
TARGET_PATH="$(/usr/bin/awk '
  $0 == "Downloads:" { found=1; next }
  found && $1 == "path:" { print substr($0, index($0, ":") + 2); exit }
  found && /^[^[:space:]]/ { exit }
' "$CONFIG_PATH")"

if [[ -z "$TARGET_PATH" || ! -d "$TARGET_PATH" ]]; then
  print -u2 -- 'ERROR: Configured Downloads target is unavailable.'
  exit 1
fi
...
if ! /bin/mkdir "$LOCK_DIR" 2>/dev/null; then
  ...
  if [[ -n "$holder_pid" ]] && /bin/kill -0 "$holder_pid" 2>/dev/null; then
    print -u2 -- "ERROR: Another Task 05 run (PID $holder_pid) holds the execution lock ($LOCK_DIR). Refusing to run concurrently. Let it finish or terminate it first."
    exit 1
  fi
...
RUN_DIR="$(/usr/bin/mktemp -d "$OUTPUT_DIR/.task05.XXXXXX")" || { /bin/rm -rf "$LOCK_DIR"; print -u2 -- 'ERROR: Cannot create repository-local staging directory.'; exit 1; }
CSV_PATH="$RUN_DIR/metadata.csv"
JSON_PATH="$RUN_DIR/metadata.json"
SUMMARY_PATH="$RUN_DIR/summary.md"
REPORT_PATH="$RUN_DIR/05_downloads_inventory.txt"
...

{
  ...
  print -- "- Engine: Task 05 Downloads metadata inventory"
  print
  section 'Folder Path'
  print -- "$TARGET_PATH"
  print
...
{
  print -- 'LifeOS Organizer — Downloads Metadata Inventory'
  print -- "Inventory ID: $INVENTORY_ID"
  print -- "Timestamp: $TIMESTAMP"
...
/bin/mv "$REPORT_PATH" "$PROJECT_DIR/reports/05_downloads_inventory.txt" || exit 1
